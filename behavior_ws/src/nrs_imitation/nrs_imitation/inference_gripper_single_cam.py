...
DEFAULT_ROS_ARGS = [
    ...
    "-p", "gripper_goal_current_topic:=/gripper/goal_current_mA",
    "-p", "control_hz:=125.0",
    "-p", "infer_hz:=10.0",
    ...
    "-p", "force_history_len:=30",
    "-p", "use_gripper_history:=true",
    "-p", "gripper_history_len:=15",
    "-p", "gripper_history_hz:=30.0",
    "-p", "gripper_history_sync_slop_sec:=0.020",
    "-p", "gripper_history_max_age_sec:=0.20",
    "-p", "gripper_history_debug_every_n:=30",
    ...
    "-p", "gripper_goal_current_min_mA:=0.0",
    "-p", "gripper_goal_current_max_mA:=1345.0",
    "-p", "gripper_goal_current_deadband_mA:=5.0",
